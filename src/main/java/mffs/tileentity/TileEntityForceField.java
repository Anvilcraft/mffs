...
public class TileEntityForceField extends TileEntityAdvanced {
  ...
  public Packet getDescriptionPacket() {
    if (this.getProjector() != null) {
      ...
      if (this.camoStack != null) {
        nbt.setTag("projector",
                   this.projector.writeToNBT(new NBTTagCompound()));
  ...
  public void onDataPacket(NetworkManager arg0,
    ...
    if (nbt.hasKey("itemID") && nbt.hasKey("itemMetadata")) {
      this.camoStack = new ItemStack(Item.getItemById(nbt.getInteger("itemID")),
                                     1, nbt.getInteger("itemMetadata"));
    }
  ...
  public TileEntityForceFieldProjector getProjectorSafe() {
    if (this.projector != null &&
        ...
        (this.getWorldObj().isRemote ||
         ((TileEntityForceFieldProjector)this.projector.getTileEntity(
              (IBlockAccess)this.getWorldObj()))
             .getCalculatedField()
             .contains(new Vector3(this)))) {
      return (TileEntityForceFieldProjector)this.projector.getTileEntity(
          (IBlockAccess)this.getWorldObj());
    }
    return null;
  }
