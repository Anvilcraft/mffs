...
public class ManipulatorCalculationThread extends Thread {
  ...
  public ManipulatorCalculationThread(
      final TileEntityForceManipulator projector) {
    this.manipulator = projector;
  }

  public ManipulatorCalculationThread(final TileEntityForceManipulator
                                          projector,
                                      final IThreadCallBack callBack) {
    this(projector);
  ...
  public void run() {
    ...
    try {
      final Set<Vector3> mobilizationPoints =
          this.manipulator.getInteriorPoints();
