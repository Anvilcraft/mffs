...
public class TileEntityForceManipulator extends TileEntityFieldInteraction {
    ...
    public void updateEntity() {
        ...
        if (this.getMode() != null && Settings.ENABLE_MANIPULATOR) {
            if (!this.getWorldObj().isRemote && this.manipulationVectors != null &&
                ...
                final ForgeDirection dir = this.getDirection((IBlockAccess) this.getWorldObj(), this.xCoord,
                ...
                for (final Vector3 position : this.manipulationVectors) {
                    if (this.moveBlock(position, dir) && this.isBlockVisible(position) &&
                        ...
                        nbtList.appendTag(
                                (NBTBase) position.writeToNBT(new NBTTagCompound()));
                        ++i;
                    }
                }
            ...
            if (this.isActive() && super.ticks % 20L == 0L &&
                ...
                if (this.getModuleCount(ModularForceFieldSystem.itemModuleSilence,
                        new int[0]) <= 0) {
                ...
                if (super.ticks % 120L == 0L && !super.isCalculating &&
                    ...
                    for (final Vector3 position2 : this.getInteriorPoints()) {
                        if (this.isBlockVisible(position2) &&
                                (this.displayMode == 2 ||
                                        position2.getBlock((IBlockAccess) this.getWorldObj()) != Blocks.air)
                                &&
                                j < Settings.MAX_FORCE_FIELDS_PER_TICK) {
                            nbtList2.appendTag(
                                    (NBTBase) position2.writeToNBT(new NBTTagCompound()));
                            ++j;
                        }
                    }
    ...
    public void onFxsPacket(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < nbtList.tagCount(); ++i) {
            final Vector3 vector = Vector3.readFromNBT(nbtList.getCompoundTagAt(i)).add(0.5);
            if (type == 1) {
                ModularForceFieldSystem.proxy.renderHologram(
                        this.getWorldObj(), vector, 1.0f, 1.0f, 1.0f, 30,
                        vector.clone().modifyPositionFromSide(
                                this.getDirection((IBlockAccess) this.getWorldObj(), this.xCoord,
                                        this.yCoord, this.zCoord)));
            } else if (type == 2) {
                ModularForceFieldSystem.proxy.renderHologram(
                        this.getWorldObj(), vector, 1.0f, 0.0f, 0.0f, 30,
                        vector.clone().modifyPositionFromSide(
                                this.getDirection((IBlockAccess) this.getWorldObj(), this.xCoord,
                                        this.yCoord, this.zCoord)));
                this.updatePushedObjects(0.022f);
    ...
    @Override
    public void onReceivePacket(PacketTile.Type type,
            final NBTTagCompound dataStream) {
    ...
    public int getFortronCost() {
        return (int) ((super.getFortronCost() + this.anchor.getMagnitude()) *
                1000.0);
    }
    ...
    protected boolean canMove() {
        ...
        final ForgeDirection dir = this.getDirection((IBlockAccess) this.getWorldObj(), this.xCoord,
                this.yCoord, this.zCoord);
        Label_0033: for (final Vector3 position : mobilizationPoints) {
            if (position.getBlock((IBlockAccess) this.getWorldObj()) != Blocks.air) {
                ...
                final TileEntity tileEntity = position.getTileEntity((IBlockAccess) this.getWorldObj());
                ...
                final Vector3 targetPosition = position.clone().modifyPositionFromSide(dir);
                if (targetPosition.getTileEntity((IBlockAccess) this.getWorldObj()) == this) {
                    return false;
                }
                ...
                final Block blockID = targetPosition.getBlock((IBlockAccess) this.getWorldObj());
                if (blockID != Blocks.air &&
                        (!blockID.isReplaceable(this.getWorldObj(), targetPosition.intX(),
                                targetPosition.intY(),
                                targetPosition.intZ()) &&
    ...
    protected boolean moveBlock(final Vector3 position,
            final ForgeDirection direction) {
        if (!this.getWorldObj().isRemote) {
            final Vector3 newPosition = position.clone().modifyPositionFromSide(direction);
            final TileEntity tileEntity = position.getTileEntity((IBlockAccess) this.getWorldObj());
    ...
    public void updatePushedObjects(final float amount) {
        ...
        if (axisalignedbb != null) {
            final List<Entity> entities = this.getWorldObj().getEntitiesWithinAABB(Entity.class, axisalignedbb);
            for (final Entity entity : entities) {
                entity.addVelocity((double) (amount * dir.offsetX),
    ...
    public AxisAlignedBB getSearchAxisAlignedBB() {
        final Vector3 positiveScale = new Vector3(this)
                .add(this.getTranslation())
                .add(this.getPositiveScale());
        final Vector3 negativeScale = new Vector3(this)
                .add(this.getTranslation())
                .subtract(this.getNegativeScale());
        final Vector3 minScale = new Vector3(Math.min(positiveScale.x, negativeScale.x),
                Math.min(positiveScale.y, negativeScale.y),
                Math.min(positiveScale.z, negativeScale.z));
        final Vector3 maxScale = new Vector3(Math.max(positiveScale.x, negativeScale.x),
                Math.max(positiveScale.y, negativeScale.y),
                Math.max(positiveScale.z, negativeScale.z));
        return AxisAlignedBB.getBoundingBox(
                (double) minScale.intX(), (double) minScale.intY(),
                (double) minScale.intZ(), (double) maxScale.intX(),
                (double) maxScale.intY(), (double) maxScale.intZ());
    }
